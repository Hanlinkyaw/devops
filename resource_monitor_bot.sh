...
cpuThreshold=75
ramThreshold=75
diskThreshold=80 # Set threshold to 20% for disk usage of the / directory
...
if (( $(echo "$cpu > $cpuThreshold" | bc -l) )); then
    sendMessage=true
    message+="%0A*🖥️ CPU Usage*: $cpu% (Threshold: $cpuThreshold%)%0A"
...
if (( $(echo "$ram_usage_percentage > $ramThreshold" | bc -l) )); then
    ...
    message+="Top 10 processes by RAM usage (in MB):%0A\`\`\`%0A$topRamProcesses%0A\`\`\`"
fi
